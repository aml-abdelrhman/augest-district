...
export default auth((req) => {
  try {
    ...
    const isAuthRoute =
      firstRouteSegment === "auth" || pathname.includes("/auth/");
    ...
    const isProtectedRoute =
      ...
      !isPublicRoute &&
      !isAuthRoute &&
      PROTECTED_ROUTES.has(firstRouteSegment);
...
export const config = {
  ...
  matcher: [
    ...
    "/((?!api|_next/static|_next/image|favicon.ico|.*\\.svg$|.*\\.png$|.*\\.jpg$|.*\\.jpeg$|.*\\.ttf$|.*\\.otf$|.*\\.woff$|.*\\.woff2$).*)",
  ],
};
